...
def avgHeadwayNew(rSchedule, periodDF = None):
    '''
    ...
    sched = sf.orderTrips(sched)

    # Convert arr_time to datettime type
    sched['arr_time'] = \
        sched.arr_time.apply(lambda x: x if pd.isna(x) else dt.datetime.strptime(x, '%I:%M %p'))
    ...
    if not periodDF is None:
        ...
        if rSchedule['direction_id'].all() == 'Inbound':
            period_df = sched[sched.Period == 1].reset_index(drop = True)
        elif rSchedule['direction_id'].all() == 'Outbound':
            period_df = sched[sched.Period == 2].reset_index(drop = True)
        else:
            print('Warning: Need directionality to compute peak hdwy')
